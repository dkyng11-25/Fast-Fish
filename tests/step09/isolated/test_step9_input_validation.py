"""
Step 9 Input Validation Test (Isolated Synthetic)
==================================================

Validates that Step 9 correctly reads from generic symlinks and handles missing inputs.

This test ensures Step 9 follows the input reading pattern:
...
def test_step9_reads_from_generic_clustering_symlink(tmp_path):
    """
    Verify Step 9 reads from generic clustering symlink.
    ...
    Verify:
    - Step 9 succeeds by reading from generic symlink
    - Step 9 loads clustering results correctly
    """
    # Create sandbox
    ...
    # Verify Step 9 loaded clustering from generic symlink
    assert "Cluster file:" in result.stdout, \
        "Step 9 should log which cluster file it's using"
    assert "clustering_results_spu.csv" in result.stdout, \
        "Step 9 should use the generic symlink"
    
    # Step 9 may complete or fail during processing, but it should have loaded the cluster file
    print(f"\n✅ Step 9 successfully read from generic clustering symlink")
    if result.returncode != 0:
        print(f"   ⚠️ Note: Step 9 failed during processing (exit code {result.returncode})")
        print(f"   This is separate from input file reading")


def test_step9_fails_gracefully_when_clustering_missing(tmp_path):
    """
    Verify Step 9 fails with clear error when clustering results are missing.
    ...
    Verify:
    - Step 9 fails with non-zero exit code
    ...
    # Verify Step 9 failed
    assert result.returncode != 0, \
        "Step 9 should fail when clustering results are missing"
    ...
    print(f"\n✅ Step 9 correctly failed with error when clustering missing")


def test_step9_uses_fallback_clustering_file(tmp_path):
    """
    Verify Step 9 uses fallback clustering file when primary is missing.
    ...
    Verify:
    - Step 9 succeeds using fallback
    - Step 9 logs which file it's using
    """
    # Create sandbox
    sandbox = create_sandbox(tmp_path)
    output_dir = sandbox / "output"
    ...
    # Verify Step 9 used the fallback file
    assert "Cluster file:" in result.stdout, \
        "Step 9 should log which cluster file it's using"
    assert "enhanced_clustering_results.csv" in result.stdout, \
        "Step 9 should use the fallback clustering file"
    
    print(f"\n✅ Step 9 successfully used fallback clustering file")
    if result.returncode != 0:
        print(f"   ⚠️ Note: Step 9 failed during processing (exit code {result.returncode})")
