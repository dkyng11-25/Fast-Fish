"""
Step 30 Dual Output Pattern Test (Isolated)
====================================================

Tests that Step 30 creates timestamped files + symlinks.
...
def test_step30_dual_output_isolated(tmp_path):
    """Test Step 30 creates timestamped files + symlinks in isolation."""
    ...
    try:
        ...
        pytest.skip(f"Step 30 requires more setup: {e}")
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 30 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 30."""
...
def _run_step30(sandbox: Path):
    """Run Step 30 in isolated sandbox."""
    ...
    timestamped_file = output_dir / f"step30_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step30_output.csv"
