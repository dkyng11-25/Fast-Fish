#!/usr/bin/env python3
"""
Step 20 Validation Runner

Handles validation of data validation from Step 20 of the pipeline.
...
def validate_step20_data_validation(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 20 data validation output.
    ...
    """
    logger.info(f"Starting Step 20 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"data_validation_{period}.csv",
            'summary_output': f"data_validation_summary_{period}.csv",
            'log_file': f"data_validation_log_{period}.log"
        }
        ...
        logger.info(f"Step 20 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 20 validation: {str(e)}")
...
def run_step20_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 20 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step20_data_validation(period)
...
if __name__ == "__main__":
    ...
    results = run_step20_validation(period)
    
    print(f"Step 20 validation results:")
