#!/usr/bin/env python3
"""
Step 33 Validation Runner

Handles validation of store level merchandising rules from Step 33 of the pipeline.
...
def validate_step33_store_level_merchandising_rules(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 33 store level merchandising rules output.
    ...
    """
    logger.info(f"Starting Step 33 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"store_level_merchandising_rules_{period}.csv",
            'summary_output': f"store_level_merchandising_rules_summary_{period}.csv",
            'log_file': f"store_level_merchandising_rules_log_{period}.log"
        }
        ...
        logger.info(f"Step 33 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 33 validation: {str(e)}")
...
def run_step33_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 33 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step33_store_level_merchandising_rules(period)
...
if __name__ == "__main__":
    ...
    results = run_step33_validation(period)
    
    print(f"Step 33 validation results:")
