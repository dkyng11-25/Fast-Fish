#!/usr/bin/env python3
"""
Step 32 Validation Runner

Handles validation of store allocation from Step 32 of the pipeline.
...
def validate_step32_store_allocation(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 32 store allocation output.
    ...
    """
    logger.info(f"Starting Step 32 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"store_allocation_{period}.csv",
            'summary_output': f"store_allocation_summary_{period}.csv",
            'log_file': f"store_allocation_log_{period}.log"
        }
        ...
        logger.info(f"Step 32 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 32 validation: {str(e)}")
...
def run_step32_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 32 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step32_store_allocation(period)
...
if __name__ == "__main__":
    ...
    results = run_step32_validation(period)
    
    print(f"Step 32 validation results:")
