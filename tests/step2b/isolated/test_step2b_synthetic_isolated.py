...
def _write_minimal_inputs_to_sandbox(sandbox: Path, period: str):
    """Write minimal input files to sandbox data/api_data directory."""
    api_data = sandbox / "data" / "api_data"
    api_data.mkdir(parents=True, exist_ok=True)
    
    # Minimal inputs expected by Step 2B - write to data/api_data where Step 2b looks for them
    (api_data / f"complete_category_sales_{period}.csv").write_text(
        "str_code,sub_cate_name,sal_amt\nS0001,CatX,10\n",
        encoding="utf-8",
    )
    (api_data / f"complete_spu_sales_{period}.csv").write_text(
        "str_code,spu_code\nS0001,X1\n",
        encoding="utf-8",
    )
    (api_data / f"store_config_{period}.csv").write_text(
        "str_code,store_name\nS0001,Demo Store\n",
        encoding="utf-8",
    )
