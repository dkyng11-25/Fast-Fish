...
def get_output_files(analysis_level='spu', yyyymm=None, period=None):
    period_label = f"{{yyyymm}}{{period}}" if yyyymm and period else "{PERIOD_LABEL}"
    return {{
...
""".strip()
    ...
    # Mock sell_through_validator.py for Fast Fish validation
    validator_stub = """
import pandas as pd
...
class SellThroughValidator:
    ...
    def validate(self, spu_code, category, subcategory, cluster_id):
        # Mock validation - always return reasonable sell-through
        return {
            'is_valid': True,
            'predicted_sell_through': 0.75,
            'confidence': 0.8,
            'historical_avg': 0.70,
        }
...
def load_historical_data_for_validation():
    ...
    return pd.DataFrame([
        {'spu_code': 'SPU001', 'category': 'T恤', 'avg_sell_through': 0.75},
        {'spu_code': 'SPU002', 'category': 'T恤', 'avg_sell_through': 0.70},
        {'spu_code': 'SPU003', 'category': '休闲裤', 'avg_sell_through': 0.80},
    ])
""".strip()
