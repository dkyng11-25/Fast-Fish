...
def get_step_input(step_name, output_key, yyyymm=None, period=None):
    """Mock get_step_input for testing"""
    if yyyymm and period:
        return f"output/enhanced_fast_fish_format_{yyyymm}{period}.csv"
    return "output/enhanced_fast_fish_format_202510A.csv"
...
def _load_step17_outputs(sandbox: Path) -> dict:
    ...
    # Look for Step 17 output files (actual naming pattern)
    augmented_files = list(output_dir.glob("fast_fish_with_historical_and_cluster_trending_analysis_*.csv"))
...
class TestStep17Synthetic:
    ...
    def test_step17_execution_with_synthetic_data(self, tmp_path):
        ...
        # List files created
        output_files = list((sandbox / "output").glob("*"))
        print(f"\nFiles in output/: {[f.name for f in output_files]}")
        ...
        assert len(outputs) > 0, f"Step 17 should create output files. Found files: {[f.name for f in output_files]}"
