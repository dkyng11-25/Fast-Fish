"""
Step 8 Input Validation Test (Isolated Synthetic)
==================================================

Validates that Step 8 correctly reads from generic symlinks and handles missing inputs.

This test ensures Step 8 follows the input reading pattern:
- Reads from output/clustering_results_spu.csv (generic symlink) - REQUIRED
- Has fallback chain for clustering files
- Reads from API data files (quantity/sales data)
...
def create_synthetic_quantity_data(output_path, num_records=100):
    """Create synthetic quantity/sales data."""
    quantity_data = pd.DataFrame({
        ...
        'sal_qty': [10 + i for i in range(num_records)],
        'sal_amt': [100 + i * 10 for i in range(num_records)]
    })
    quantity_data.to_csv(output_path, index=False)


def test_step8_reads_from_generic_clustering_symlink(tmp_path):
    """
    Verify Step 8 reads from generic clustering symlink.
    
    Setup:
    - Create timestamped clustering file
    - Create generic symlink pointing to it
    - Create required quantity data
    - Run Step 8
    
    Verify:
    - Step 8 succeeds by reading from generic symlink
    - Step 8 loads clustering results correctly
    """
    # Create sandbox
    ...
    # Create required quantity data
    quantity_file = api_data_dir / "complete_spu_sales_202510A.csv"
    create_synthetic_quantity_data(quantity_file, num_records=100)
    
    print(f"\n📁 Created test setup:")
    print(f"   Clustering (timestamped): {timestamped_cluster.name}")
    print(f"   Clustering (generic symlink): clustering_results_spu.csv")
    print(f"   Quantity data: complete_spu_sales_202510A.csv")
    ...
    # Verify Step 8 loaded clustering from generic symlink
    assert "Using clustering results:" in result.stdout, \
        "Step 8 should log which cluster file it's using"
    assert "clustering_results_spu.csv" in result.stdout, \
        "Step 8 should use the generic symlink"
    
    # Step 8 may complete or fail during processing, but it should have loaded the cluster file
    print(f"\n✅ Step 8 successfully read from generic clustering symlink")
    if result.returncode != 0:
        print(f"   ⚠️ Note: Step 8 failed during processing (exit code {result.returncode})")
        print(f"   This is separate from input file reading")


def test_step8_fails_gracefully_when_clustering_missing(tmp_path):
    """
    Verify Step 8 fails with clear error when clustering results are missing.
    
    Setup:
    - Create quantity data but NO clustering file
    - Run Step 8
    
    Verify:
    - Step 8 fails with non-zero exit code
    ...
    # Verify Step 8 failed
    assert result.returncode != 0, \
        "Step 8 should fail when clustering results are missing"
    ...
    print(f"\n✅ Step 8 correctly failed with error when clustering missing")


def test_step8_uses_fallback_clustering_file(tmp_path):
    """
    Verify Step 8 uses fallback clustering file when primary is missing.
    
    Setup:
    - Create fallback clustering file (clustering_results.csv)
    - Do NOT create primary file
    - Run Step 8
    
    Verify:
    - Step 8 succeeds using fallback
    - Step 8 logs which file it's using
    """
    # Create sandbox
    sandbox = create_sandbox(tmp_path)
    output_dir = sandbox / "output"
    ...
    fallback_cluster = output_dir / "clustering_results.csv"
    ...
    print(f"   Fallback clustering: YES (clustering_results.csv)")
    print(f"   Quantity data: YES")
    ...
    # Verify Step 8 used the fallback file
    assert "Using clustering results:" in result.stdout, \
        "Step 8 should log which cluster file it's using"
    assert "clustering_results.csv" in result.stdout, \
        "Step 8 should use the fallback clustering file"
    
    print(f"\n✅ Step 8 successfully used fallback clustering file")
    if result.returncode != 0:
        print(f"   ⚠️ Note: Step 8 failed during processing (exit code {result.returncode})")
