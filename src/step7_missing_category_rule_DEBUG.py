...
def identify_missing_opportunities_with_sellthrough(sales_df: pd.DataFrame, cluster_df: pd.DataFrame, 
            ...
            if avg_sales_per_store >= MIN_OPPORTUNITY_VALUE:
                ...
                if should_approve:
                    ...
                    if USE_ROI:
                        ...
                        # DEBUG: Log ROI calculation details with FULL PRECISION
                        # Check if this is near the margin_uplift threshold
                        near_threshold = abs(margin_uplift - MIN_MARGIN_UPLIFT) < 20  # Within $20 of threshold
                        
                        if near_threshold:
                            print(f"\n{'='*80}")
                            print(f"THRESHOLD EDGE CASE: store={store_code}, feature={feature_name}, cluster={cluster_id}")
                            print(f"{'='*80}")
                            print(f"INPUTS:")
                            print(f"  median_amt (raw): {median_amt} (type: {type(median_amt).__name__})")
                            print(f"  avg_sales_per_store (raw): {avg_sales_per_store} (type: {type(avg_sales_per_store).__name__})")
                            print(f"  unit_price (raw): {unit_price} (type: {type(unit_price).__name__})")
                            print(f"  margin_rate: {mr_used} (type: {type(mr_used).__name__})")
                            print()
                            print(f"CALCULATION STEPS:")
                            print(f"  1. expected_units = max(1.0, ceil((median_amt * SCALING_FACTOR) / unit_price))")
                            print(f"     = max(1.0, ceil(({median_amt} * {SCALING_FACTOR}) / {unit_price}))")
                            print(f"     = max(1.0, ceil({median_amt * SCALING_FACTOR / max(1e-6, unit_price)}))")
                            print(f"     = {expected_units}")
                            print()
                            print(f"  2. unit_cost = unit_price * (1 - margin_rate)")
                            print(f"     = {unit_price} * (1 - {mr_used})")
                            print(f"     = {unit_cost}")
                            print()
                            print(f"  3. margin_per_unit = unit_price - unit_cost")
                            print(f"     = {unit_price} - {unit_cost}")
                            print(f"     = {margin_per_unit}")
                            print()
                            print(f"  4. margin_uplift = margin_per_unit * expected_units")
                            print(f"     = {margin_per_unit} * {expected_units}")
                            print(f"     = {margin_uplift}")
                            print(f"     (FULL PRECISION: {margin_uplift:.20f})")
                            print()
                            print(f"THRESHOLD CHECK:")
                            print(f"  margin_uplift >= MIN_MARGIN_UPLIFT?")
                            print(f"  {margin_uplift:.20f} >= {MIN_MARGIN_UPLIFT:.20f}?")
                            print(f"  Difference: {margin_uplift - MIN_MARGIN_UPLIFT:.20f}")
                            print(f"  Result: {margin_uplift >= MIN_MARGIN_UPLIFT}")
                            print()
                            print(f"OTHER CHECKS:")
                            print(f"  roi={roi_value:.20f} >= {ROI_MIN_THRESHOLD}? {roi_value >= ROI_MIN_THRESHOLD}")
                            print(f"  n_comparables={n_comparables} >= {MIN_COMPARABLES}? {n_comparables >= MIN_COMPARABLES}")
                            print()
                            
                            # Test rounding sensitivity
                            print(f"ROUNDING SENSITIVITY TEST:")
                            for decimals in [0, 1, 2, 3, 4, 5]:
                                rounded_margin = round(margin_uplift, decimals)
                                passes = rounded_margin >= MIN_MARGIN_UPLIFT
                                print(f"  Round to {decimals} decimals: ${rounded_margin:.{decimals}f} -> {'PASS' if passes else 'FAIL'}")
                            print()
                        
                        if not (roi_value >= ROI_MIN_THRESHOLD and margin_uplift >= MIN_MARGIN_UPLIFT and n_comparables >= MIN_COMPARABLES):
                            if near_threshold:
                                print(f"  ❌ FILTERED OUT (near threshold!)")
                                print(f"{'='*80}\n")
                            continue
                        
                        if near_threshold:
                            print(f"  ✅ APPROVED (near threshold!)")
                            print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"DEBUG SUMMARY: Legacy Step 7 Opportunity Creation")
    print(f"{'='*80}")
    print(f"Total opportunities created: {len(opportunities_df)}")
    if len(opportunities_df) > 0:
        print(f"Margin uplift stats: min=${opportunities_df['margin_uplift'].min():.2f}, "
              f"mean=${opportunities_df['margin_uplift'].mean():.2f}, "
              f"max=${opportunities_df['margin_uplift'].max():.2f}")
        print(f"ROI stats: min={opportunities_df['roi'].min():.2%}, "
              f"mean={opportunities_df['roi'].mean():.2%}, "
              f"max={opportunities_df['roi'].max():.2%}")
        print(f"n_comparables stats: min={opportunities_df['n_comparables'].min()}, "
              f"mean={opportunities_df['n_comparables'].mean():.1f}, "
              f"max={opportunities_df['n_comparables'].max()}")
    print(f"{'='*80}\n")
