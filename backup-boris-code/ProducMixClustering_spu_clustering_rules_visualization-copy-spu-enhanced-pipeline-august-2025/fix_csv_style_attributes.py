#!/usr/bin/env python3
"""
Fix CSV Style Attributes Using Real Data
========================================
...
This script uses the real store_config_data.csv to reconstruct complete style tags.
"""
...
def load_store_config_data() -> pd.DataFrame:
    """Load the store config data with complete attributes."""
    print("📊 Loading store config data with complete attributes...")
    
    config_file = "data/data/api_data/store_config_data.csv"
    if not os.path.exists(config_file):
        raise FileNotFoundError(f"Store config file not found: {config_file}")
    
    config_df = pd.read_csv(config_file)
    print(f"✅ Loaded {len(config_df):,} store config records")
    
    # Show sample of the data
    print(f"📋 Sample attributes from store config:")
    sample = config_df.iloc[0]
    print(f"   Season: {sample['season_name']}")
    print(f"   Gender: {sample['sex_name']}")
    print(f"   Location: {sample['display_location_name']}")
    print(f"   Category: {sample['big_class_name']}")
    print(f"   Subcategory: {sample['sub_cate_name']}")
    
    return config_df

def create_attribute_mapping(config_df: pd.DataFrame) -> Dict[str, Dict]:
    """Create mapping from category combinations to their most common attributes."""
    print("🗺️  Creating attribute mapping from store config data...")
    ...
    for name, group in config_df.groupby(grouping_cols):
    ...
    for i, (key, attrs) in enumerate(list(category_mapping.items())[:3]):
        print(f"   {i+1}. {key} → {attrs['season']}, {attrs['gender']}, {attrs['location']}")
...
def translate_attributes(attributes: Dict[str, str]) -> Dict[str, str]:
    ...
    category_map = {
        ...
        '裙/连衣裙': 'Skirts', '外套': 'Outerwear', '防晒衣': 'Sun Protection',
        '配饰': 'Accessories', '袜': 'Socks', '茄克': 'Jackets',
        '套装': 'Sets', '内衣': 'Underwear', '卫衣': 'Hoodies'
    }
...
def reconstruct_style_tags(current_csv: pd.DataFrame, category_mapping: Dict[str, Dict]) -> pd.DataFrame:
    ...
    def create_complete_style_tag(current_tags: str) -> str:
        ...
        if category_key in category_mapping:
            ...
            # Try with just the big_class - find any subcategory for this class
            ...
            if fallback_keys:
                ...
                # Use intelligent defaults based on category type
                if any(word in big_class for word in ['袜', '配饰', '鞋']):
                    location = '鞋配'
                elif any(word in big_class for word in ['内衣', '家居']):
                    location = '后场'
                else:
                    location = '前台'
                ...
                # Default attributes
                attributes = {
                    'season': '夏',  # Summer for June B
                    'gender': '中',  # Unisex default
                    'location': location,
                    'big_class': big_class,
                    'sub_cate': sub_cate
                }
    ...
    print(f"   ✅ Matched with real data: {matched_records:,} ({matched_records/total_records*100:.1f}%)")
    print(f"   🔄 Used intelligent defaults: {default_records:,} ({default_records/total_records*100:.1f}%)")
...
def save_corrected_csv(corrected_csv: pd.DataFrame) -> str:
    ...
    has_attributes = any(part in sample_tag for part in ['Summer', 'Spring', 'Autumn', 'Winter']) and \
                    any(part in sample_tag for part in ['Men', 'Women', 'Unisex']) and \
                    any(part in sample_tag for part in ['Front-store', 'Back-store', 'Shoes-Accessories'])
...
def verify_improvement(original_csv: pd.DataFrame, corrected_csv: pd.DataFrame):
    """Verify the improvement in style tag completeness."""
    print("\n🔍 Verifying improvement...")
    
    # Check original format
    original_sample = original_csv['Target_Style_Tags'].iloc[0]
    corrected_sample = corrected_csv['Target_Style_Tags'].iloc[0]
    
    print(f"📋 Before: {original_sample}")
    print(f"📋 After:  {corrected_sample}")
    
    # Count attributes
    original_parts = len(str(original_sample).split('|'))
    corrected_parts = len([p for p in str(corrected_sample).split(',') if p.strip()])
    
    print(f"📊 Attribute count: {original_parts} → {corrected_parts}")
    print(f"✅ Improvement: +{corrected_parts - original_parts} attributes per record")

def main():
    """Main execution function."""
    print("🚀 Fixing CSV Style Attributes Using Real Data")
    print("=" * 60)
    ...
    try:
        ...
        # Load store config data with complete attributes
        config_data = load_store_config_data()
        
        # Create attribute mapping from real data
        category_mapping = create_attribute_mapping(config_data)
        
        # Reconstruct complete style tags
        corrected_csv = reconstruct_style_tags(current_csv, category_mapping)
        
        # Verify improvement
        verify_improvement(current_csv, corrected_csv)
        ...
        print("🔗 Based on real store configuration data")
