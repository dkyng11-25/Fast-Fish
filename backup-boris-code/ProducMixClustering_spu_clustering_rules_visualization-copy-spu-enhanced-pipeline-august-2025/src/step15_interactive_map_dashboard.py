...
def get_store_spu_violations(store_code: str, spu_details: Dict[str, pd.DataFrame]) -> Dict:
        ...
        if rule_name == 'rule7_missing_opportunities':
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'issue': f"🚨 MISSING HIGH-PERFORMER: {spu_code} worth ${opportunity_value:,.0f} opportunity",
                    'action': f"✅ ADD {spu_code} - {pct_stores_selling*100:.1f}% of cluster peers sell this ({stores_selling}/{cluster_size} stores)",
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'severity': severity,
                    'business_impact': 'NEGATIVE',
                    'issue': f"⚖️ ALLOCATION IMBALANCE: {imbalance_type} by {abs(row['adjustment_needed']):.1f} styles (Z-score: {row['z_score']:.2f})",
                    'action': f"🔄 {'REDUCE' if imbalance_type == 'OVER_ALLOCATED' else 'INCREASE'} allocation to {row['suggested_allocation']:.1f} styles to match cluster performance",
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'business_impact': 'NEGATIVE',
                    'issue': f"📉 INSUFFICIENT INVENTORY: {spu_code} has {row['style_count']:.3f} < {row['recommended_target']:.3f} minimum",
                    'action': f"📈 INCREASE {spu_code} by {row['increase_needed']:.3f} units to meet viable threshold ({category_details})",
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'business_impact': 'NEGATIVE',
                    'issue': f"💸 OVERCAPACITY: {spu_code} current {current_qty:.1f}, reduce {abs(qty_change):.1f}",
                    'action': f"🔻 REDUCE {spu_code} by {abs(qty_change):.1f} units to cut waste",
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'business_impact': 'NEGATIVE',
                    'issue': f"💰 MISSED SALES: {spu_code} increase by {qty_increase:.1f} units",
                    'action': f"📈 INCREASE {spu_code} by {qty_increase:.1f} units",
            ...
            for _, row in store_data.iterrows():
                ...
                spu_info = {
                    ...
                    'business_impact': 'NEGATIVE',
                    'issue': f"📊 UNDERPERFORMANCE: {spu_code} increase by {qty_increase:.1f} units",
                    'action': f"📈 INCREASE {spu_code} by {qty_increase:.1f} units",
    ...
    if len(all_spus) > 0:
        ...
        store_violations['action_items'] = [
            f"🚨 {len(high_priority)} HIGH PRIORITY business issues need immediate attention",
            f"📈 {len(all_spus)} total optimization opportunities across {len(rules_count)} business rules",
            f"🎯 Top SPU issues: {', '.join([spu['spu_code'] for spu in all_spus[:3]])}",
            f"💰 Estimated business impact: {total_financial_impact:.0f} units/styles optimization potential",
            f"🔧 Key actions: Add missing SPUs, rebalance allocations, increase facings, focus sales efforts"
        ]
        
        # Add interpretation note
        store_violations['business_note'] = "⚠️ All flagged rules represent BUSINESS PROBLEMS that need resolution. Higher violation counts indicate more optimization opportunities."
...
def load_map_data() -> Tuple[pd.DataFrame, Dict]:
    ...
    real_coordinates_file = 'output/stores_with_feels_like_temperature.csv'
    fallback_coordinates_file = os.path.join(DATA_DIR, 'store_coordinates_extended.csv')
    
    if os.path.exists(real_coordinates_file) and os.path.exists(fallback_coordinates_file):
...
def load_trendiness_analysis() -> Dict:
    """Load trendiness analysis data"""
    try:
        with open('output/production_trendiness_analysis.json', 'r') as f:
            return json.load(f)
    except Exception as e:
        log_progress(f"Warning: Could not load trendiness analysis: {e}")
        return {}
...
def generate_store_popup_content(row: pd.Series, violations_count: int = None, rule_values: List[int] = None, enhanced_data: Dict = None) -> str:
    ...
    <div style="font-family: Arial, sans-serif; min-width: 350px;">
        <h3 style="color: #2c3e50; margin-bottom: 10px;">🏪 Store {row['str_code']}</h3>
        ...
        <div style="background: #fff3cd; padding: 8px; border-radius: 4px; margin: 10px 0;">
            <div style="font-size: 0.85em; font-weight: 600; color: #856404; margin-bottom: 4px;">💰 Total Revenue Opportunity:</div>
            <div style="font-size: 1.1em; font-weight: bold; color: #856404;">{_format_currency(total_revenue_opportunity)}</div>
        </div>
        
        <p><strong>📍 Location:</strong> {row.get('latitude', 'N/A'):.4f}, {row.get('longitude', 'N/A'):.4f}</p>
        <p><strong>🏭 Cluster:</strong> {row.get('Cluster', 'N/A')}</p>
        <p><strong>⚠️ Total Violations:</strong> {violation_count}</p>
        
        {f'<p style="margin-top: 8px;"><strong>🚨 Rules Violated:</strong> {", ".join(violations)}</p>' if violations else '<p style="margin-top: 8px; color: #27ae60;"><strong>✅ No violations detected</strong></p>'}
        ...
        <div style="margin-top: 15px; padding-top: 10px; border-top: 1px solid #bdc3c7;">
            <button onclick="showEnhancedStoreAnalysis('{row['str_code']}')" 
                ...
                🔍 Enhanced SPU Analysis & Recommendations
            </button>
            <button onclick="showTrendinessDetails('{row['str_code']}')" 
                ...
                📈 Trendiness Details ({trendiness_recs} recommendations)
            </button>
        </div>
    </div>
    """
        ...
        for store_code in map_data['str_code'].unique():
            ...
            if store_row is not None:
                total_rule_violations = sum([
                    int(store_row.get('rule7_missing_category', 0)),
                    int(store_row.get('rule8_imbalanced_spu', 0)),
                    int(store_row.get('rule9_below_minimum', 0)),
                    int(store_row.get('rule10_smart_overcapacity_strict', 0)),
                    int(store_row.get('rule11_missed_sales_opportunity', 0)),
                    int(store_row.get('rule12_sales_performance', 0))
                ])
...
<body>
    ...
    <script>
            ...
            const modalContent = `
                <div class="modal-overlay" id="analysisModal">
                    <div class="modal-content" style="max-width: 1200px;">
                        <button class="modal-close" onclick="closeEnhancedAnalysis()">×</button>
                        
                        <h2 style="color: #2c3e50; margin-bottom: 20px;">🏪 Store ${{store.str_code}} - Enhanced SPU Analysis</h2>
                        
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                            <div style="background: #ecf0f1; padding: 15px; border-radius: 8px;">
                                <h3 style="color: #34495e; margin-bottom: 10px;">📍 Store Information</h3>
                            ...
                            <div style="background: #ecf0f1; padding: 15px; border-radius: 8px;">
                                <h3 style="color: #34495e; margin-bottom: 10px;">📊 Cluster Context</h3>
                            ...
                            <div style="background: #e8f5e8; padding: 15px; border-radius: 8px;">
                                <h3 style="color: #27ae60; margin-bottom: 10px;">🎯 SPU Summary</h3>
                        ...
                        <h3 style="color: #34495e; margin-bottom: 15px;">📋 Rule-by-Rule Analysis (Click to see SPUs)</h3>
                ...
                if (spuData.data_note) {{
                    return `
                        <div style="background: #fff3cd; padding: 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #ffc107;">
                            <h3 style="color: #856404; margin-bottom: 10px;">⚠️ Data Availability Notice</h3>
                    ...
                    return `
                        <div style="background: #d5f4e6; padding: 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #27ae60;">
                            <h3 style="color: #27ae60; margin-bottom: 10px;">✅ All SPUs Performing Well</h3>
            ...
            return `
                <div style="background: #fff3cd; padding: 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #ffc107;">
                    <div style="display: flex; justify-content: between; align-items: center; margin-bottom: 15px;">
                        <h3 style="color: #856404; margin: 0;">🎯 Cumulative SPU Analysis (Worst Offenders First)</h3>
        ...
        function generateEnhancedRuleCard(rule, storeCode, spuData) {{
            ...
            const statusIcon = rule.violated ? '🔴' : '✅';
        ...
        function generateSPUDrillDown(ruleCode, spus) {{
            ...
            return `
                <div style="margin-top: 15px; border-top: 1px solid #ddd; padding-top: 15px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        <strong style="color: #2c3e50;">📦 Affected SPUs (Worst First):</strong>
        ...
        function generateActionItems(ruleCode) {{
            const actions = {{
                'rule7': ['• Analyze top-performing SPUs in cluster peers', '• Consider adding missing high-value SPUs', '• Review category gaps vs cluster standards'],
                'rule8': ['• Rebalance allocation based on cluster patterns', '• Focus resources on high-performing categories', '• Reduce investment in underperforming areas'],
                'rule9': ['• Increase SPU variety to meet standards', '• Add complementary products in key categories', '• Review minimum viable SKU requirements'],
                'rule10': ['• Identify overcapacity areas for optimization', '• Reallocate resources to growth opportunities', '• Consider downsizing underperforming sections'],
                'rule11': ['• Focus on categories with highest sales gaps', '• Implement targeted promotional strategies', '• Analyze successful peer store tactics'],
                'rule12': ['• Review overall performance strategies', '• Benchmark against cluster leaders', '• Implement performance improvement plans']
            }};
            
            const actionList = actions[ruleCode] || ['• Contact support for specific recommendations'];
                ...
                return `
                    <div class="cluster-item" onclick="selectCluster(${{clusterId}})">
                        ...
                        <div class="cluster-stats">
                            ${{cluster.store_count}} stores • ${{cluster.total_violations}} total violations
                            ${{clusterFinancial ? `<br><strong>💰 ${{formatCurrency(totalFinancialImpact)}} opportunity</strong>` : ''}}
                        </div>
            ...
            const modalContent = `
                <div class="modal-overlay" id="clusterAnalysisModal">
                    <div class="modal-content" style="max-width: 1400px;">
                        <button class="modal-close" onclick="closeClusterAnalysis()">×</button>
                        
                        <h2 style="color: #2c3e50; margin-bottom: 20px;">🏭 Cluster ${{clusterId}} - Financial Analysis & Store Performance</h2>
                        
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                            <div style="background: #e8f5e8; padding: 15px; border-radius: 8px; text-align: center;">
                                <h3 style="color: #27ae60; margin-bottom: 10px;">💰 Revenue Opportunity</h3>
                            ...
                            <div style="background: #fff3cd; padding: 15px; border-radius: 8px; text-align: center;">
                                <h3 style="color: #856404; margin-bottom: 10px;">💸 Cost Savings</h3>
                            ...
                            <div style="background: #f8d7da; padding: 15px; border-radius: 8px; text-align: center;">
                                <h3 style="color: #721c24; margin-bottom: 10px;">📊 Investment Needed</h3>
                            ...
                            <div style="background: #d1ecf1; padding: 15px; border-radius: 8px; text-align: center;">
                                <h3 style="color: #0c5460; margin-bottom: 10px;">📈 ROI Estimate</h3>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 25px;">
                            <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                                <h3 style="color: #343a40; margin-bottom: 15px;">🏪 Cluster Overview</h3>
                ...
                <div><strong>Total Impact:</strong> ${{formatCurrency(clusterFinancial.cluster_performance_metrics.total_financial_impact)}}</div>
                            ...
                            <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                                <h3 style="color: #343a40; margin-bottom: 15px;">🎯 Implementation Priority</h3>
                        ...
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                            <h3 style="color: #343a40; margin-bottom: 15px;">🏆 Top Opportunity Stores (Ranked by Financial Impact)</h3>
                            <div style="max-height: 400px; overflow-y: auto;">
                                     ...
                                     <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; margin: 8px 0; background: white; border-radius: 8px; border-left: 4px solid ${{getImpactColor(store.total_revenue_opportunity + store.total_cost_savings)}}; cursor: pointer;" onclick="showEnhancedStoreAnalysis('${{store.store_code}}'); closeClusterAnalysis();">
                                         <div>
                                             ...
                                             <div style="font-size: 0.9em; color: #6c757d;">${{store.total_spu_issues}} SPU issues • ${{store.rules_affected}} rules affected</div>
                                         </div>
                                         <div style="text-align: right;">
            ...
            const modalContent = `
                <div class="modal-overlay" id="trendinessModal">
                    <div class="modal-content" style="max-width: 900px;">
                        <button class="modal-close" onclick="closeTrendinessDetails()">×</button>
                        
                        <h2 style="color: #2c3e50; margin-bottom: 20px;">📈 Store ${{storeCode}} - Trendiness Analysis</h2>
                        
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 25px;">
                            <div style="background: #ecf0f1; padding: 20px; border-radius: 10px;">
                                <h3 style="color: #34495e; margin-bottom: 15px;">🏪 Product Mix Analysis</h3>
                                <div style="margin-bottom: 10px;">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                            ...
                            <div style="background: #e8f5e8; padding: 20px; border-radius: 10px;">
                                <h3 style="color: #27ae60; margin-bottom: 15px;">⚖️ Balance Assessment</h3>
                        ...
                        <div style="background: #fff3cd; padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 5px solid #ffc107;">
                            <h3 style="color: #856404; margin-bottom: 15px;">💡 Trendiness Recommendations (${{storeAnalysis.recommendations.length}} items)</h3>
                        ...
                        <div style="background: #d5f4e6; padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 5px solid #27ae60;">
                            <h3 style="color: #27ae60; margin-bottom: 15px;">📊 Sales Performance Ratios</h3>
...
def main() -> None:
    ...
    try:
        ...
        log_progress(f"✅ Interactive Map Dashboard saved to {OUTPUT_FILE}")
        log_progress(f"🗺️ Enhanced Dashboard Features ({ANALYSIS_LEVEL.upper()} Level):")
        log_progress(f"   • Interactive map with {summary_stats['total_stores']:,} store locations")
        log_progress(f"   • Color-coded markers by violation severity (0-6 violations)")
        log_progress(f"   • Detailed {ANALYSIS_LEVEL.upper()}-level popups with rule-specific insights")
        log_progress(f"   • SPU-level violations analysis (worst offenders first)")
        log_progress(f"   • Combined rules breakdown for individual stores")
        log_progress(f"   • Sales team action items and recommendations")
        log_progress(f"   • Rule-based filtering (8 filter options: All + Rules 7-12)")
        log_progress(f"   • Cluster and violation count filtering")
        log_progress(f"   • Geographic distribution visualization")
        log_progress(f"   • Real-time statistics panel with {ANALYSIS_LEVEL.title()}-level metrics")
        if ANALYSIS_LEVEL == "spu":
            log_progress(f"   • Enhanced SPU-level granularity with {len(spu_details)} detailed rule analyses")
        
    except Exception as e:
        log_progress(f"❌ Error generating map dashboard: {str(e)}")
        raise
