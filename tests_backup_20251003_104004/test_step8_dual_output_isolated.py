"""
Step 8 Dual Output Pattern Test (Isolated)
====================================================

Tests that Step 8 creates timestamped files + symlinks.
...
def test_step8_dual_output_isolated(tmp_path):
    """Test Step 8 creates timestamped files + symlinks in isolation."""
    ...
    try:
        ...
        pytest.skip(f"Step 8 requires more setup: {e}")
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 8 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 8."""
...
def _run_step8(sandbox: Path):
    """Run Step 8 in isolated sandbox."""
    ...
    timestamped_file = output_dir / f"step8_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step8_output.csv"
