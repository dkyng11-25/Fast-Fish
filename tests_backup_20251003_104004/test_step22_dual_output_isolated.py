...
def test_step22_dual_output_isolated(tmp_path):
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 22 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 22."""
...
def _run_step22(sandbox: Path):
    ...
    timestamped_file = output_dir / f"step22_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step22_output.csv"
