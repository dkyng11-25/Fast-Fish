#!/usr/bin/env python3
"""
Step 31 Validation Runner

Handles validation of gap analysis workbook from Step 31 of the pipeline.
...
def validate_step31_gap_analysis_workbook(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 31 gap analysis workbook output.
    ...
    """
    logger.info(f"Starting Step 31 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"gap_analysis_workbook_{period}.csv",
            'summary_output': f"gap_analysis_workbook_summary_{period}.csv",
            'log_file': f"gap_analysis_workbook_log_{period}.log"
        }
        ...
        logger.info(f"Step 31 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 31 validation: {str(e)}")
...
def run_step31_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 31 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step31_gap_analysis_workbook(period)
...
if __name__ == "__main__":
    ...
    results = run_step31_validation(period)
    
    print(f"Step 31 validation results:")
