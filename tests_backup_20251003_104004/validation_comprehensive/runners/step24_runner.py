#!/usr/bin/env python3
"""
Step 24 Validation Runner

Handles validation of comprehensive cluster labeling from Step 24 of the pipeline.
...
def validate_step24_comprehensive_cluster_labeling(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 24 comprehensive cluster labeling output.
    ...
    """
    logger.info(f"Starting Step 24 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"comprehensive_cluster_labeling_{period}.csv",
            'summary_output': f"comprehensive_cluster_labeling_summary_{period}.csv",
            'log_file': f"comprehensive_cluster_labeling_log_{period}.log"
        }
        ...
        logger.info(f"Step 24 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 24 validation: {str(e)}")
...
def run_step24_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 24 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step24_comprehensive_cluster_labeling(period)
...
if __name__ == "__main__":
    ...
    results = run_step24_validation(period)
    
    print(f"Step 24 validation results:")
