#!/usr/bin/env python3
"""
Step 21 Validation Runner

Handles validation of label tag recommendations from Step 21 of the pipeline.
...
def validate_step21_label_tag_recommendations(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 21 label tag recommendations output.
    ...
    """
    logger.info(f"Starting Step 21 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"label_tag_recommendations_{period}.csv",
            'summary_output': f"label_tag_recommendations_summary_{period}.csv",
            'log_file': f"label_tag_recommendations_log_{period}.log"
        }
        ...
        logger.info(f"Step 21 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 21 validation: {str(e)}")
...
def run_step21_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 21 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step21_label_tag_recommendations(period)
...
if __name__ == "__main__":
    ...
    results = run_step21_validation(period)
    
    print(f"Step 21 validation results:")
