#!/usr/bin/env python3
"""
Step 25 Validation Runner

Handles validation of product role classification from Step 25 of the pipeline.
...
def validate_step25_product_role_classifier(period: str = "202508A", 
                                           data_dir: str = "../output") -> Dict[str, Any]:
    """
    Validate Step 25 product role classification output.
    ...
    """
    logger.info(f"Starting Step 25 validation for period {period}")
    ...
    try:
        ...
        expected_files = {
            'main_output': f"product_role_classifier_{period}.csv",
            'summary_output': f"product_role_classifier_summary_{period}.csv",
            'log_file': f"product_role_classifier_log_{period}.log"
        }
        ...
        logger.info(f"Step 25 validation completed. Passed: {results['validation_passed']}")
        
    except Exception as e:
        logger.error(f"Error during Step 25 validation: {str(e)}")
...
def run_step25_validation(period: str = "202508A") -> Dict[str, Any]:
    """
    Run Step 25 validation.
    ...
    Returns:
        Dictionary with validation results
    """
    return validate_step25_product_role_classifier(period)
...
if __name__ == "__main__":
    ...
    results = run_step25_validation(period)
    
    print(f"Step 25 validation results:")
