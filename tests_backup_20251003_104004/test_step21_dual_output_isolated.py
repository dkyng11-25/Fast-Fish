...
def test_step21_dual_output_isolated(tmp_path):
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 21 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 21."""
...
def _run_step21(sandbox: Path):
    ...
    timestamped_file = output_dir / f"step21_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step21_output.csv"
