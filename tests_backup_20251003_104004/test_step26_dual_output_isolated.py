...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 26."""
...
def _run_step26(sandbox: Path):
    """Run Step 26 in isolated sandbox."""
    ...
    timestamped_file = output_dir / f"step26_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step26_output.csv"
