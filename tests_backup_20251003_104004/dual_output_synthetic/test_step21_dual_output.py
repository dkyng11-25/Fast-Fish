"""
Step 21 Dual Output Pattern Test
==================================================

Tests that Step 21 (Label Tag Recommendations) creates output files WITHOUT timestamps.
...
def test_step21_creates_output_without_timestamp(tmp_path):
    """Test that Step 21 creates output files WITHOUT timestamp in filename"""
    ...
    pattern = "D_F_Label_Tag_Recommendation_Sheet_{PERIOD_LABEL}.xlsx"
    expected_file = output_dir / pattern
    
    # Look for files matching the base pattern
    matching_files = list(output_dir.glob(f"D_F_Label_Tag_Recommendation_Sheet_*.xlsx"))
    
    if not matching_files:
        pytest.skip(f"No output files found for step 21 - run pipeline first")
    ...
    print(f"✅ Step 21 output files have correct naming (no timestamps)")
    for file in matching_files:
        print(f"   - {file.name}")


def test_step21_output_has_period_label(tmp_path):
    """Test that Step 21 output files have period labels"""
    output_dir = PROJECT_ROOT / "output"
    ...
    if not matching_files:
        pytest.skip(f"No output files found for step 21")
    ...
    print(f"✅ Step 21 output files have period labels")


def test_step21_output_consumable_by_downstream(tmp_path):
    """Test that Step 21 output can be found by downstream steps"""
    output_dir = PROJECT_ROOT / "output"
    ...
    print(f"✅ Step 21 output is consumable by downstream steps")
