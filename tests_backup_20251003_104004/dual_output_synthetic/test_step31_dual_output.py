"""
Step 31 Dual Output Pattern Test
==================================================

Tests that Step 31 (Gap Analysis Workbook) creates output files WITHOUT timestamps.
...
def test_step31_creates_output_without_timestamp(tmp_path):
    """Test that Step 31 creates output files WITHOUT timestamp in filename"""
    ...
    pattern = "gap_analysis_workbook_{PERIOD_LABEL}.xlsx"
    expected_file = output_dir / pattern
    
    # Look for files matching the base pattern
    matching_files = list(output_dir.glob(f"gap_analysis_workbook_*.xlsx"))
    
    if not matching_files:
        pytest.skip(f"No output files found for step 31 - run pipeline first")
    ...
    print(f"✅ Step 31 output files have correct naming (no timestamps)")
    for file in matching_files:
        print(f"   - {file.name}")


def test_step31_output_has_period_label(tmp_path):
    """Test that Step 31 output files have period labels"""
    output_dir = PROJECT_ROOT / "output"
    ...
    if not matching_files:
        pytest.skip(f"No output files found for step 31")
    ...
    print(f"✅ Step 31 output files have period labels")


def test_step31_output_consumable_by_downstream(tmp_path):
    """Test that Step 31 output can be found by downstream steps"""
    output_dir = PROJECT_ROOT / "output"
    ...
    print(f"✅ Step 31 output is consumable by downstream steps")
