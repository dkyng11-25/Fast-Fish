"""
Step 8 Dual Output Pattern Test (UPDATED)
==================================================

Tests that Step 8 creates timestamped outputs with symlinks.
...
def test_step8_creates_timestamped_output():
    """Test that Step 8 creates timestamped output file"""
    ...
    if not matching_files:
        pytest.skip(f"No timestamped files found for step 8 - run pipeline first")
    
    print(f"✅ Step 8 creates timestamped outputs:")
...
def test_step8_creates_period_symlink():
    """Test that Step 8 creates period-labeled symlink"""
    output_dir = PROJECT_ROOT / "output"
    ...
    if not period_files:
        pytest.skip(f"No period-labeled files found for step 8")
    
    # Check if they are symlinks
    symlinks = [f for f in period_files if f.is_symlink()]
    
    print(f"✅ Step 8 creates period-labeled symlinks:")
...
def test_step8_symlinks_point_to_timestamped():
    ...
    if not symlinks:
        pytest.skip(f"No symlinks found for step 8")
    ...
    print(f"✅ Step 8 symlinks correctly point to timestamped files")
