"""
Step 10 Dual Output Pattern Test (UPDATED)
==================================================

Tests that Step 10 creates timestamped outputs with symlinks.
...
def test_step10_creates_timestamped_output():
    """Test that Step 10 creates timestamped output file"""
    ...
    if not matching_files:
        pytest.skip(f"No timestamped files found for step 10 - run pipeline first")
    
    print(f"✅ Step 10 creates timestamped outputs:")
...
def test_step10_creates_period_symlink():
    """Test that Step 10 creates period-labeled symlink"""
    output_dir = PROJECT_ROOT / "output"
    ...
    if not period_files:
        pytest.skip(f"No period-labeled files found for step 10")
    
    # Check if they are symlinks
    symlinks = [f for f in period_files if f.is_symlink()]
    
    print(f"✅ Step 10 creates period-labeled symlinks:")
...
def test_step10_symlinks_point_to_timestamped():
    ...
    if not symlinks:
        pytest.skip(f"No symlinks found for step 10")
    ...
    print(f"✅ Step 10 symlinks correctly point to timestamped files")
