"""
Step 6 Dual Output Pattern Test
==================================================

Tests that Step 6 (Cluster Analysis) creates output files WITHOUT timestamps.
...
def test_step6_creates_output_without_timestamp(tmp_path):
    """Test that Step 6 creates output files WITHOUT timestamp in filename"""
    ...
    pattern = "clustering_results_spu_{PERIOD_LABEL}.csv"
    expected_file = output_dir / pattern
    
    # Look for files matching the base pattern
    matching_files = list(output_dir.glob(f"clustering_results_spu_*.csv"))
    
    if not matching_files:
        pytest.skip(f"No output files found for step 6 - run pipeline first")
    ...
    print(f"✅ Step 6 output files have correct naming (no timestamps)")
    for file in matching_files:
        print(f"   - {file.name}")


def test_step6_output_has_period_label(tmp_path):
    """Test that Step 6 output files have period labels"""
    output_dir = PROJECT_ROOT / "output"
    ...
    if not matching_files:
        pytest.skip(f"No output files found for step 6")
    ...
    print(f"✅ Step 6 output files have period labels")


def test_step6_output_consumable_by_downstream(tmp_path):
    """Test that Step 6 output can be found by downstream steps"""
    output_dir = PROJECT_ROOT / "output"
    ...
    print(f"✅ Step 6 output is consumable by downstream steps")
