...
def _ensure_step13_run():
    ...
    if all(os.path.exists(_latest(g) or "") for g in [detailed_glob, cluster_glob, store_glob]):
        return
    import subprocess
    subprocess.run(["python3", "src/step13_consolidate_spu_rules.py"], check=True, env=env)
...
def test_step13_store_summary_matches_detailed():
    ...
    assert store_summary_path and os.path.exists(store_summary_path)
...
def test_step13_no_sales_new_classes_blocked():
    ...
    cmap = pd.read_csv(cpath, usecols=["str_code", "cluster_id"], dtype={"str_code": str}, low_memory=False)
