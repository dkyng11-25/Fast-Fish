"""
Step 2 Dual Output Pattern Test (Isolated)
====================================================

Tests that Step 2 creates timestamped files + symlinks.
...
def test_step2_dual_output_isolated(tmp_path):
    """Test Step 2 creates timestamped files + symlinks in isolation."""
    ...
    try:
        ...
        pytest.skip(f"Step 2 requires more setup: {e}")
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 2 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 2."""
...
def _run_step2(sandbox: Path):
    """Run Step 2 in isolated sandbox."""
    ...
    timestamped_file = output_dir / f"step2_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step2_output.csv"
