"""
Step 23 Dual Output Pattern Test (Isolated)
====================================================

Tests that Step 23 creates timestamped files + symlinks.
...
def test_step23_dual_output_isolated(tmp_path):
    """Test Step 23 creates timestamped files + symlinks in isolation."""
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 23 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 23."""
...
def _run_step23(sandbox: Path):
    ...
    timestamped_file = output_dir / f"step23_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step23_output.csv"
