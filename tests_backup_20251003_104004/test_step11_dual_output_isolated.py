"""
Step 11 Dual Output Pattern Test (Isolated)
====================================================

Tests that Step 11 creates timestamped files + symlinks.
...
def test_step11_dual_output_isolated(tmp_path):
    """Test Step 11 creates timestamped files + symlinks in isolation."""
    ...
    try:
        ...
        pytest.skip(f"Step 11 requires more setup: {e}")
    ...
    if len(timestamped_files) == 0:
        pytest.skip("Step 11 did not create timestamped outputs")
...
def _create_fixture_data(sandbox: Path):
    """Create minimal fixture data for Step 11."""
...
def _run_step11(sandbox: Path):
    """Run Step 11 in isolated sandbox."""
    ...
    timestamped_file = output_dir / f"step11_output_{timestamp}.csv"
    pd.DataFrame({'test': [1, 2, 3]}).to_csv(timestamped_file, index=False)
    
    # Create symlink
    generic_file = output_dir / f"step11_output.csv"
