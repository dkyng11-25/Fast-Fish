...
# Matrix type에 따라서 어떤 파일을 쓸지 사용한 map {SPU, Subcategory, Category-Aggregated}
MATRIX_CONFIGS = {
    "subcategory": {
    ...
    "spu": {
        "normalized": "data/normalized_spu_limited_matrix.csv", # 매출 규모가 아닌 무엇을 파는지가 비슷한 매장끼리 clustering
        "original": "data/store_spu_limited_matrix.csv", 
        "feature_name": "SPU",
        "description": "SPU-Level Clustering (Top 1000 SPUs)" # 각 매장을 상위 1000개의 SPU의 매출 기준으로 Clustering
    },
    "category_agg": {
...
# 실제 clustering에 사용할 행렬 가져오기 
CURRENT_CONFIG = MATRIX_CONFIGS[MATRIX_TYPE]
INPUT_MATRIX = CURRENT_CONFIG["normalized"] # 실제 clustering에 사용할 feature matrix 
ORIGINAL_MATRIX = CURRENT_CONFIG["original"]

# Other configuration
# 나중에 clustering할 때 온도 제약 걸 시 사용 위함 
...
# PCA configuration - adaptive based on matrix type (차원 축소)
...
# Create output directories (결과 저장 폴더 생성)
...
# ============================================================================
# IMPROVEMENT I1: Store Feature Encoding Functions
# ============================================================================
def load_store_features() -> Optional[pd.DataFrame]:
    ...
    Returns:
        DataFrame with encoded store features or None if not available
    """
    if not USE_STORE_FEATURES:
        return None
    
    try:
        if not os.path.exists(STORE_SALES_FILE):
            log_progress(f"Warning: Store sales file not found: {STORE_SALES_FILE}")
            return None
        
        log_progress(f"IMPROVEMENT I1: Loading store features from {STORE_SALES_FILE}")
        store_sales = pd.read_csv(STORE_SALES_FILE)
        ...
        # Binary encoding for str_type (流行=1, 基础=0)
        features['str_type_encoded'] = (features['str_type'] == '流行').astype(float)
        
        # Ordinal encoding for sal_type (AA=5, A=4, B=3, C=2, D=1)
        ...
        log_progress(f"IMPROVEMENT I1: Encoded store features for {len(result)} stores")
        log_progress(f"  - str_type_encoded: binary (流行=1, 基础=0)")
        log_progress(f"  - sal_type_encoded: ordinal (AA=5, A=4, B=3, C=2, D=1)")
        log_progress(f"  - traffic_normalized: min-max scaled")
        
        return result
        
    except Exception as e:
        log_progress(f"Warning: Could not load store features: {str(e)}")
        return None


def enhance_matrix_with_store_features(normalized_df: pd.DataFrame, store_features: Optional[pd.DataFrame]) -> pd.DataFrame:
    """
    IMPROVEMENT I1: Enhance the normalized SPU matrix with store features.
    
    Args:
        normalized_df: Row-normalized SPU matrix
        store_features: Encoded store features DataFrame
        
    Returns:
        Enhanced matrix with store features appended
    """
    if store_features is None or not USE_STORE_FEATURES:
        log_progress("Store features not available, using SPU matrix only")
        return normalized_df
    
    log_progress("IMPROVEMENT I1: Enhancing matrix with store features...")
    
    # Align indices
    normalized_df.index = normalized_df.index.astype(str)
    common_stores = normalized_df.index.intersection(store_features.index)
    
    if len(common_stores) == 0:
        log_progress("Warning: No common stores found, using SPU matrix only")
        return normalized_df
    
    # Filter to common stores
    enhanced = normalized_df.loc[common_stores].copy()
    store_features_common = store_features.loc[common_stores].copy()
    
    # Append store features as new columns
    for col in ['str_type_encoded', 'sal_type_encoded', 'traffic_normalized']:
        enhanced[f'STORE_{col}'] = store_features_common[col].values
    
    log_progress(f"IMPROVEMENT I1: Enhanced matrix shape: {enhanced.shape}")
    log_progress(f"  - SPU features: {normalized_df.shape[1]}")
    log_progress(f"  - Store features: 3 (str_type, sal_type, traffic)")
    log_progress(f"  - Total features: {enhanced.shape[1]}")
...
def determine_optimal_initial_clusters(pca_df: pd.DataFrame) -> int:
    """
    Determine the optimal initial number of clusters.
    
    IMPROVEMENT I2: Use fixed cluster count (30) instead of dynamic n_samples // 50.
    ...
    # IMPROVEMENT I2: Use fixed cluster count if enabled
    if USE_FIXED_CLUSTER_COUNT:
        ...
        # Original logic: Calculate number of clusters needed for exactly 50 stores per cluster
...
def main():
    ...
    try:
        ...
        if temp_df is not None and ENABLE_TEMPERATURE_CONSTRAINTS:
            ...
            # Determine optimal initial clusters (IMPROVEMENT I2: fixed count)
