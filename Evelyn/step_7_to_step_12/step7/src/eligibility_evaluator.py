...
from climate_config import (
    ...
    is_core_subcategory,  # Now imported from climate_config (configurable)
)
...
def evaluate_eligibility(
    ...
    IMPORTANT: Core subcategories (Straight-Leg, Jogger, Tapered) are ALWAYS
    eligible per client requirements E-04, W-01, I-05.
    ...
    # CORE SUBCATEGORY OVERRIDE (Per Client Requirement E-04, W-01, I-05)
    # Core subcategories must NEVER be filtered by climate/season logic
    if is_core_subcategory(category_name):
        return EligibilityResult(
            status=EligibilityStatus.ELIGIBLE,
            reason=f"Core subcategory '{category_name}' - always eligible per business requirement",
            climate_match=True,
            season_match=True,
            store_temperature=store_temperature,
            spu_temperature_band="Core",
