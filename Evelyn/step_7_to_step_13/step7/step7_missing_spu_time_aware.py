...
# Import eligibility evaluator for explicit eligibility output
from eligibility_evaluator import (
    EligibilityStatus,
    EligibilityResult,
    evaluate_eligibility,
    add_eligibility_to_dataframe,
...
def apply_climate_time_gates(
    ...
    # NEW: Add explicit eligibility_status for downstream modules (Step 8+)
    # ELIGIBLE = both gates passed, INELIGIBLE = any gate failed, UNKNOWN = insufficient data
    enhanced_df['eligibility_status'] = EligibilityStatus.UNKNOWN.value
    enhanced_df.loc[enhanced_df['final_recommendation'] == 'ADD', 'eligibility_status'] = EligibilityStatus.ELIGIBLE.value
    enhanced_df.loc[enhanced_df['final_recommendation'].isin(['SUPPRESS_TIME', 'SUPPRESS_CLIMATE']), 'eligibility_status'] = EligibilityStatus.INELIGIBLE.value
    
    # Add eligibility_reason (same as suppression_reason for ineligible, else "Eligible")
    enhanced_df['eligibility_reason'] = enhanced_df.apply(
        lambda r: r['suppression_reason'] if r['eligibility_status'] == 'INELIGIBLE' else 'Climate and season match - eligible for recommendation',
        axis=1
    )
    ...
    eligibility_summary = get_eligibility_summary(enhanced_df)
    ...
    print(f"\n📋 ELIGIBILITY STATUS (for downstream modules):")
    print(f"   ELIGIBLE: {eligibility_summary['ELIGIBLE']:,}")
    print(f"   INELIGIBLE: {eligibility_summary['INELIGIBLE']:,}")
    print(f"   UNKNOWN: {eligibility_summary['UNKNOWN']:,}")
    print(f"{'='*60}\n")
