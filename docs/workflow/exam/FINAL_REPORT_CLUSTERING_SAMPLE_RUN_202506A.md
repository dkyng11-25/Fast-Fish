...
#### What is PCA?
**Principal Component Analysis (PCA)** is a dimensionality reduction technique that transforms high-dimensional data (1,000 SPU features) into a lower-dimensional space (2D for visualization) while preserving the most important patterns.

#### How to Read This Chart

| Element | Meaning |
|---------|--------|
| **Each dot** | Represents one store (2,260 stores total) |
| **Dot color** | Indicates cluster assignment (45 different colors) |
| **X-axis (PC1)** | First principal component - captures ~12% of variance |
| **Y-axis (PC2)** | Second principal component - captures ~8% of variance |
| **Dot position** | Stores with similar sales patterns are positioned closer together |

#### Key Observations

1. **Cluster Overlap (❌ Problem)**
   - Many clusters overlap significantly in the 2D space
   - This indicates stores in different clusters have similar sales patterns
   - **Ideal:** Clusters should form distinct, separated groups

2. **Outliers at Edges**
   - Dots far from the center are outlier stores
   - These may be high-volume flagship stores or specialty locations
   - They can distort clustering if not handled properly

3. **Dense Center Region**
   - Most stores cluster in the center
   - This suggests many stores have similar, average sales patterns
   - Current clustering may be over-segmenting similar stores

#### What Good Clustering Looks Like
```
✅ Good: Distinct color groups with clear boundaries
❌ Current: Mixed colors throughout, no clear separation
```
...
![Clustering Visualizations Dashboard](./clustering_visualizations.png)

This dashboard provides a comprehensive view of clustering quality through 6 interconnected charts.

---

#### Chart 1: PCA 2D Scatter (Top-Left)

**Purpose:** Visualize how stores are distributed in reduced feature space.

| What to Look For | Current Status | Ideal Status |
|------------------|----------------|---------------|
| Cluster separation | ❌ Overlapping | ✅ Distinct groups |
| Outlier handling | ⚠️ Visible outliers | ✅ Outliers in separate clusters |
| Color distribution | ❌ Mixed | ✅ Clear color regions |

---

#### Chart 2: Cluster Size Bar Chart (Top-Center)

**Purpose:** Show how many stores are in each cluster.

**How to Read:**
- **X-axis:** Cluster ID (0-44)
- **Y-axis:** Number of stores in that cluster
- **Bar height:** Taller = more stores

**Key Observations:**

| Issue | Evidence | Impact |
|-------|----------|--------|
| **Singleton clusters** | 6 clusters have only 1 store | Wasted cluster capacity |
| **Imbalanced sizes** | Range from 1 to 187 stores | Uneven resource allocation |
| **Target missed** | Target was 50 stores/cluster | Only ~10 clusters near target |

**Ideal Distribution:**
```
✅ All bars should be approximately equal height (~50 stores each)
❌ Current: Highly variable bar heights
```

---

#### Chart 3: Silhouette Score Distribution (Top-Right)

**Purpose:** Show clustering quality for each individual store.

**What is Silhouette Score?**
- Measures how similar a store is to its own cluster vs. other clusters
- Range: -1 to +1
  - **+1:** Store is perfectly matched to its cluster
  - **0:** Store is on the boundary between clusters
  - **-1:** Store is in the wrong cluster

**How to Read:**
- **X-axis:** Silhouette score value
- **Y-axis:** Number of stores with that score
- **Distribution shape:** Should be right-skewed (most scores > 0.5)

**Current Status:**

| Metric | Value | Interpretation |
|--------|-------|----------------|
| Mean score | 0.0812 | ❌ Very poor (target ≥ 0.5) |
| Stores with score < 0 | ~30% | ❌ Many stores misclassified |
| Stores with score > 0.5 | ~5% | ❌ Very few well-clustered stores |

---

#### Chart 4: Per-Cluster Average Silhouette (Bottom-Left)

**Purpose:** Compare clustering quality across different clusters.

**How to Read:**
- **X-axis:** Cluster ID
- **Y-axis:** Average silhouette score for that cluster
- **Bar color:** Often indicates quality (green=good, red=poor)

**Key Observations:**

| Cluster Quality | Count | Examples |
|-----------------|-------|----------|
| ✅ Good (> 0.25) | 3 clusters | Clusters 37, 41, 10 |
| ⚠️ Fair (0.1-0.25) | 5 clusters | Clusters 9, 27 |
| ❌ Poor (< 0.1) | 37 clusters | Most clusters |

**What This Means:**
- Only 3 out of 45 clusters have acceptable quality
- 82% of clusters have poor internal cohesion
- Stores within most clusters are not truly similar

---

#### Chart 5: Top SPU Heatmap by Cluster (Bottom-Center)

**Purpose:** Show which products define each cluster.

**How to Read:**
- **Rows:** Clusters (0-44)
- **Columns:** Top SPU codes (product identifiers)
- **Color intensity:** Darker = higher sales of that SPU in that cluster

**Key Observations:**

| Pattern | Meaning | Current Status |
|---------|---------|----------------|
| **Distinct rows** | Each cluster has unique top products | ❌ Many similar rows |
| **Vertical stripes** | Same products dominate all clusters | ⚠️ Some visible |
| **Diagonal pattern** | Good cluster differentiation | ❌ Not present |

**Top SPUs Appearing Across All Clusters:**
- `15K1030` - Casual pants (束脚裤)
- `15K1053` - Casual pants (束脚裤)
- `15T1076` - T-shirt (短袖T恤)
- `15K1036` - Casual pants

**Problem:** Same products dominate most clusters, indicating clustering is based on sales volume rather than product mix differences.

---

#### Chart 6: Metrics Summary (Bottom-Right)

**Purpose:** Display key clustering quality metrics.

| Metric | Value | Target | Status |
|--------|-------|--------|--------|
| **Silhouette Score** | 0.0812 | ≥ 0.5 | ❌ FAIL |
| **Calinski-Harabasz** | 57.02 | Higher = better | ⚠️ Low |
| **Davies-Bouldin** | 2.89 | < 1.0 | ❌ FAIL |
| **Number of Clusters** | 45 | Optimal TBD | ⚠️ May be too many |

**Metric Explanations:**

1. **Silhouette Score (0.0812)**
   - Measures cluster cohesion and separation
   - Current value indicates clusters are barely better than random
   - **Action needed:** Improve feature engineering

2. **Calinski-Harabasz Index (57.02)**
   - Ratio of between-cluster to within-cluster variance
   - Higher is better; current value is relatively low
   - **Action needed:** Reduce number of clusters or improve features

3. **Davies-Bouldin Index (2.89)**
   - Measures average similarity between clusters
   - Lower is better; values > 1 indicate overlapping clusters
   - **Action needed:** Current clusters are too similar to each other
